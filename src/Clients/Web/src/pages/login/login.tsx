...
class Login extends React.Component<any, any> {
    ...
    async login(values: any) {
        ...
        try {
            ...
            message.info("正在取得登录凭证");
            let response = await login(model);
            let token = response.data.access_token;

            let curTime = new Date();
            var expire = new Date(curTime.setSeconds(curTime.getSeconds() + response.data.expires_in));

            // 保存登录信息
            StorageService.setAccessToken(token);
            StorageService.setExpire(expire.toString());

            // 获取用户姓名以及权限信息
            message.info("正在取得用户权限");
            let userInfoResponse = await getUserInfo();
            StorageService.setUserName(userInfoResponse.data.data.userName);
            StorageService.setRights(userInfoResponse.data.data.identities);
